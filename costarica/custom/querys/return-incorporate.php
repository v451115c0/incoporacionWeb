		...
		else
		{
			...
			if($done['type'] == 1 && $done['kit'] == "5006" || $done['type'] == 1 && $done['kit'] == "5023" || $done['type'] == 1 && $done['kit'] == "5024" || $done['type'] == 1 && $done['kit'] == "5025" || $done['type'] == 1 && $done['kit'] == "5026")
			{
				...
				//echo "1///https://nikkenlatam.com/interno/carrito-compras/login-integration-incorporate.php?email=" . base64_encode($email);
				 echo "1///https://nikkenlatam.com/interno/carrito-compras/login-integration-incorporate.php?email=" . base64_encode($email) . '&item=' . $done['kit'] . "&item2=" . $done['playera'];
			}
			elseif($done['type'] == 0)
			{
				/*Enviar a arma tu entorno En este caso en especial de costarica se enviará a la T.V. con un autologin para que el usuario proceda a comprar*/
